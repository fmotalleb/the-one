...
type Service struct {
	...
	// actions that must pass before start of this service
	Requirements []option.Optional[string] `mapstructure:"requires,omitempty" yaml:"requires"`
	// actions that if available must be exited before start of this service
	After []option.Optional[string] `mapstructure:"After,omitempty" yaml:"After"`
	// services that rely on execution of this service
	// this will be mapped to [After] of those services and this field will be emptied before execution
